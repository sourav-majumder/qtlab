:: qtlibs.bat
:: Starts an IPython shell with the QTlab libraries loaded (on Windows). Remember to add an environment variable called PYTHONPATH that includes "[path_to_qtlab]\source".
::
...
IF EXIST "%PYTHON_PATH%\scripts\ipython.py" (
    start Console -w "QTLab" -r "/k %PYTHON_PATH%\python.exe %PYTHON_PATH%\scripts\ipython.py -gthread -p sh source/qtlibs_shell.py -- %*"
    GOTO EOF
)
:: check if version >= 0.11
IF EXIST "%PYTHON_PATH%\scripts\ipython-script.py" (
    start Console -w "QTLab" -r "/k %PYTHON_PATH%\python.exe %PYTHON_PATH%\scripts\ipython-script.py --gui=gtk -i source/qtlibs_shell.py -- %*"
    GOTO EOF
)
:: check if EPD
IF EXIST "%PYTHON_PATH%\scripts\ipython.bat" (
    start Console -w "QTLab" -r "/k %PYTHON_PATH%\scripts\ipython.bat -i source/qtlibs_shell.py"
    GOTO EOF
)

echo Failed to run qtlibs.bat
pause
:EOF
