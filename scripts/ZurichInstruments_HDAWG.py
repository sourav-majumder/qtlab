...
class HDAWG8Exception(Exception):
	pass

class HDAWG8NotExactException(Exception):
	pass

class ZurichInstruments_HDAWG(object):
	'''
	This is a driver with subroutines for Zurich Instruments UHFLI Lock in Amplifier.
	'''
	def __init__(self, device_id, reset=False):
		apilevel = 6
		err_msg = "This can only be ran on either a HDAWG8 with the PC option enabled."
		(daq, device, props) = zhinst.utils.create_api_session(device_id, apilevel, required_devtype='HDAWG8',required_options=['PC'], required_err_msg=err_msg)
