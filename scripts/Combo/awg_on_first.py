...
def optimize_ph(ph_arr=np.linspace(-180, 180, 720), plot = False):
	...
	for index, phase in enumerate(ph_arr):
		...
		uhf.setup_awg(awg_program(4*32, 32, 1, 1, phase))
	...
	uhf.setup_awg(awg_program(4*32, 32, 1, 1, ph_arr[indices[0][0]]))
	uhf.awg_on(single=False)
	if plot:
...
def optimize_amplitude(plot=False):
	...
	q_arr = np.linspace(0, 700e-3, 10)/750e-3
...
def optimize_all(plot=False):
	uhf.setup_awg(awg_program(9000, 4500, 1, 1))
...
def awg_program(samples, width_samples, i_amplitude, q_amplitude, i_phase=0):
	awg_program_string = """
	...
	(samples, float(samples)/2, width_samples,
		uhf.awg_sine(mix_freq, samples=samples, amplitude = i_amplitude, phase = i_phase),
		uhf.awg_sine(mix_freq, samples=samples, amplitude = q_amplitude))
	return awg_program_string

def just_optimize(center_freq, mix_freq, sideband='left')
	global center_freq, mix_freq, sideband
