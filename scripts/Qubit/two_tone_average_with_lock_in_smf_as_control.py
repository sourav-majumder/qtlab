...
aps.set_frequency(measurement_frequency)
...
for no_of_mp,control_power in enumerate(control_power_array):
	...
	for control_frequency in control_frequency_array:
		...
		data.add_data_point(control_power, control_frequency,np.mean(sample['x']),np.mean(sample['y']),np.mean(sample['R']),np.mean(sample['phi']))
		index +=1
