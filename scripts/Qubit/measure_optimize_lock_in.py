...
measurement_frequency_start = 7.84967*GHz-1*MHz #at SMF
measurement_frequency_stop = 7.84967*GHz+1*MHz #at SMF
measurement_frequency_numpoints = 51
...
def generate_meta_file(no_of_measurement_frequency, measurement_frequency):
    metafile = open('%s.meta.txt' % data.get_filepath()[:-4], 'w')
    metafile.write('#inner loop\n%s\n%s\n%s\n%s\n'%
            (record_length,  start_time/us, stop_time/us, 'Time(us)'))
    metafile.write('#outer loop\n%s\n%s\n%s\n%s\n'%
            (no_of_measurement_frequency, measurement_frequency, measurement_frequency_array[0], 'Hz'))
...
areas = []
for no_of_measurement_frequency, measurement_frequency in enumerate(measurement_frequency_array):
	...
	measurement_freq_arr = np.linspace(measurement_frequency, measurement_frequency, record_length)
	time_array, voltages = rte.get_data(channels)
	voltages.append((voltages[0]**2 + voltages[1]**2)**0.5)
	data.add_data_point(measurement_freq_arr, time_array, *voltages)
	generate_meta_file(no_of_measurement_frequency+1, measurement_frequency)
	measurement_frequency_progress_bar.update(no_of_measurement_frequency+1)
