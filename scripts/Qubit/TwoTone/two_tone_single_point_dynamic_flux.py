...
def generate_meta_file():
    ...
    metafile.write('#outer loop\n%s\n%s\n%s\n%s\n'%
            (current_numpoints, stop_current, start_current, 'A'))
...
# diff=50*Hz
drive_start_freq = 9.58*GHz
drive_stop_freq = 9.62*GHz
resolution = 0.2*MHz
drive_numpoints = int(round(abs(drive_stop_freq - drive_start_freq)/resolution + 1))
drive_start_power = 10

start_current = -0.690e-3
stop_current = -0.390e-3 
current_numpoints = 51
...
current_array = np.linspace(start_current, stop_current, current_numpoints)
...
for curr in current_array:
	start_time = time.time()
	qs.sweep_current(curr)
	curr_list = np.linspace(curr, curr, num=drive_numpoints)
	traces=[[],[],[],[]]
	...
	end_time = time.time()
	data.add_data_point(curr_list, drive_freq_array, traces[0], traces[1], traces[2], traces[3])
	generate_meta_file()
