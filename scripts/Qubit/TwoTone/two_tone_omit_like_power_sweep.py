...
def generate_meta_file():
    ...
    metafile.write('#outer loop\n%s\n%s\n%s\n%s\n'%
            (sideband_freq_numpoint, sideband_stop_freq, sideband_start_freq, 'dBm'))
...
drive_start_freq = 4.652*GHz
drive_stop_freq = 4.662*GHz
resolution = 0.1*MHz
drive_numpoints = int(abs(drive_stop_freq - drive_start_freq)/resolution + 1)
sideband_start_freq = 4.65716*GHz - 6*MHz
sideband_stop_freq = 4.65716*GHz - 3*MHz
sideband_resolution =  3*MHz
sideband_freq_numpoint = int(abs(sideband_stop_freq - sideband_start_freq)/resolution + 1)
sideband_power = 0
...
for side_freq in sideband_freq_array:
    ...
    power_list = np.linspace(side_freq, side_freq, num=drive_numpoints)
    traces=[[],[],[],[]]
