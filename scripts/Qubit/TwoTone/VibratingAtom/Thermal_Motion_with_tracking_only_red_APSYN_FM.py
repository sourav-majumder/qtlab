...
# How many traces
no_of_traces = 200
target_value = -74.5 - 1.0
target_value_2 = -92 - 1.0
...
def data_file(pw):
    data=qt.Data(name='_red_drive_-30dc_fm_apy_syn_-45dBm_blue_200_traces')
...
# fsv.set_centerfrequency(smf_freq)
qt.msleep(1.0)
...
for pw in pw_list:
    ...
    while value < no_of_traces:
        ...
        # fsv.run_single()
        # trace2= fsv.get_data()
        ls = fsv.get_max_freqs(1)
        measure_peak_2=ls[1][0]
        measure_curr = qs.get_level()*1e3
        print(str(measure_peak_2)+'  '+str(measure_curr))
        mp = np.append(mp, measure_peak_2)
        curr_array = np.append(curr_array, measure_curr)
        if measure_peak_2 > target_value_2:
            data.add_data_point(value_array, trace1[0], trace1[1])
            value = value + 1
        else:
            cw_setup()
