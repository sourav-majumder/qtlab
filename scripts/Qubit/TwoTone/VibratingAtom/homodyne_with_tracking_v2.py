...
fm_start=7*MHz
fm_stop=9*MHz
fm_resolution=fm_span
fm_step=(fm_stop-fm_start)/fm_resolution+1
fm_list=np.arange(fm_start,fm_stop,fm_resolution)
...
out_meta = [fm_list[0],fm_list[-1] , len(fm_list), 'n']
once = True
...
for idx, vv in enumerate(fm_list):
    trace = []
    # cw_setup()
    ...
    # fm_c = predict_fm(vv)
    trace = mechanical(vv)
    vg_dummy = np.linspace(idx, idx, fm_numpoints)
    fm_array = np.linspace(vv-fm_span/2, vv+fm_span/2, fm_numpoints)
