...
def data_file(filename):
    ...
    data.add_coordinate('VNA power', units='dBm')
...
out_meta = [vna_power_stop, vna_power_start, power_numpoints, 'dBm']
once = True

vna_power_array = np.linspace(vna_power_start, vna_power_stop, power_numpoints)
...
for pw in vna_power_array:
    ...
    # fm_c = predict_fm()
    trace = mechanical(fm_center,pw)
    VNA_power_dummy = np.linspace(pw, pw, fm_numpoints)
    fm_array = np.linspace(fm_center-fm_span/2, fm_center+fm_span/2, fm_numpoints)
    data.add_data_point(VNA_power_dummy, fm_array, np.abs(trace), np.angle(trace))
    mp = np.append(mp, measure_peak)
    measure_curr = qs.get_level()*1e3
    # pw_list = np.append(pw_list, pw)
