...
#Outerloop
outer_meta_info = "DG FREQ"
outer_start = 6.60131*MHz - 30.
outer_stop  = 6.60131*MHz + 30.
outer_resolution = 1*Hz
outer_len = int(abs(outer_stop - outer_start)/outer_resolution + 1)
DG_drive_power= 10 #Volt

# Inner most loop
inner_meta_info = "VNA` freq" 
inner_start = 5.93*GHz
inner_stop = 6.01*GHz #center + span/2.
inner_resolution =.2*MHz
...
if_bw = 10
vna_power = -6  # 30 dB att + 1 dB cab;e + 6 dir coupler
...
## DG READY
dg.write(':SOUR1:FREQ %f'%outer_start) 
...
for outer_index, outer_value in enumerate(outerloop):
    ...
    dg.write(':SOUR1:FREQ %f'%outer_value)
    qt.msleep(0.25)
