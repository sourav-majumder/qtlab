...
x, y = np.loadtxt(r'D:\data\20191016\182437_power_sweep_dressed_cavity_6.025GHZ\pw_fr.dat', unpack= True, usecols = (0,1))
...
def predict_freq(pw):
    ...
    predicted_freq = y[idx]
    print('Cavity drive freq  '+str(predicted_freq))
    # prediction = spec_power + 20*np.log10(wp - 5.996)
    return predicted_freq*MHz


# How many traces
no_of_traces = 100
target_value = -76.6 - 1.0
...
def data_file(pw):
    data=qt.Data(name=str(pw)+'_30_Volt_thermal_motion_amplification')
...
for pw in pw_list:
    ...
    smf_freq=predict_freq(pw-12) #calculated additional attenuation is 12
    smf.set_frequency(smf_freq)
    print(str(smf_freq)+'and'+str(pw))
    qt.msleep(1.0)
    data = data_file(pw)
    value = 0
    mp = []
