...
outer_start = 0
outer_stop  = 15*us/(4.4*ns)  # 4.4*3 us 
outer_len = 401
...
if i_should_optimize:
    ...
    uhf.set('awgs/0/outputs/0/amplitude', 16/100.*max_i_amp)
    uhf.set('awgs/0/outputs/1/amplitude', 16/100.*max_q_amp)
# phase = -1.710238;
...
for outermost_index, outermost_value in enumerate(outermostloop):
    ...
    for outer_index, outer_value in enumerate(outerloop):
        ...
        uhf.setup_awg(awg_program(phase = phase, delay=outer_value))
        uhf.awg_on(single=False)
