...
def sideband_diff(sideband='left'):
	qt.msleep(0.1)
...
def optimize_amp_phase(i_arr, ph_arr, plot=False):
	diffs = []
	import progressbar
	progress_bar = progressbar.ProgressBar(maxval=len(i_arr), \
	...
	uhf.setup_awg(awg_program(ph_arr[indices[1][0]]))
	uhf.awg_on(single=False)
	if plot:
		plt.imshow(diffs, aspect='auto', extent=[ph_arr[0], ph_arr[-1], i_arr[-1], i_arr[-0]])
		plt.show()
	return i_arr[indices[0][0]],ph_arr[indices[1][0]]

def optimize_amplitude_phase(plot=False):
	i_arr = np.linspace(0, 700e-3, 10)/750e-3
	ph_arr = np.linspace(-np.pi, np.pi, 20)
	opt_i, opt_ph = optimize_amp_phase(i_arr, ph_arr, plot)
	# Fine sweep
	...
	ph_arr = (np.linspace(opt_ph-0.032, opt_ph+0.032, 20))
	return optimize_amp_phase(i_arr, ph_arr, plot)
...
def optimize_all(plot=False):
	...
	i_amp, phase = optimize_amplitude_phase(plot=plot)
	q_amp = 700e-3/750e-3
	freqs, powers = fsv.get_max_freqs(2)
...
def awg_program(phase=0):
	...
	wave w2 = sine(128, 1, %f, 6);
	wave w3 = sine(128, 1, 0, 6);	
	while (true) {
	  playWave(w2, w3);
	}""" % \
	(phase)
