...
def generate_meta_file(no_of_pulse, pulse):
	metafile = open('%s.meta.txt' % data.get_filepath()[:-4], 'w')
	metafile.write('#inner loop\n%s\n%s\n%s\n%s\n'%
			(record_length,  start_time/us, stop_time/us, 'Time(us)'))
	metafile.write('#outer loop\n%s\n%s\n%s\n%s\n'%
			(no_of_pulse, pulse, pulse_array[0], 'Pulse Length'))
...
def awg_program(pulse_length, power_points=power_array[0], sigma=gauss_width):
	awg_program_string = """
	const marker_pos = 0;
	const cycle = 4.4e-9;
	const us = 1e-6;
...
pulse_progress_bar = progressbar.ProgressBar(maxval=pulse_numpoints, \
	widgets=['Total: ', progressbar.Bar('=', '[', ']'), ' ', progressbar.Percentage(), ' (', progressbar.ETA(), ') '])
pulse_progress_bar.start()


for no_of_pulse, pulse in enumerate(pulse_array):
	uhf.setup_awg(awg_program(pulse))
	uhf.awg_on(single=False)
	# aps.set_frequency(control)
	rte.reset_averages()
	rte.run_nx_single(wait=True)
	pulse_arr = np.linspace(pulse, pulse, record_length)
	time_array, voltages = rte.get_data(channels)
	voltages.append((voltages[0]**2+voltages[1]**2)**0.5)
	data.add_data_point(pulse_arr, time_array, *voltages)
	generate_meta_file(no_of_pulse+1, pulse)
	pulse_progress_bar.update(no_of_pulse+1)
