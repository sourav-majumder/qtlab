...
uhf.set('triggers/out/1/source', 9)
uhf.set('triggers/out/0/source', 8)
uhf.set('awgs/0/time', 3)
uhf.setup_demod(demod_index = 1, order = 1, timeconstant = lock_in_time_constant)
uhf.setup_auxout(aux_index = 1, output=0, demod = 1, scale = 20e3)
uhf.setup_auxout(aux_index = 2, output=1, demod = 1, scale = 20e3)
uhf.setup_auxout(aux_index = 3, output=2, demod = 1, scale = 1)
# uhf.setup_auxout(aux_index = 4, output=3, demod = 1, scale = 1e-3)
uhf.trig_out_on(1)
uhf.imp_50()
...
def awg_program(delay, control_pulse = 22500, measurement_pulse = 225*12):
	awg_program_string = """
	wave w_left = marker(%d, 1);
	wave w_gap = marker(%d, 0);
	wave w_right = marker(%d, 2);
	wave w_marker = join(w_left, w_gap, w_right);
	while (true) {
		playWave(1, w_marker);
		wait(200000);
	}
