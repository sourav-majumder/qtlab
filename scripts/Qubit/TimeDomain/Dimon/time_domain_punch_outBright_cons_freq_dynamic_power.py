...
def generate_meta_file():
    ...
    metafile.write('#inner loop\n%s\n%s\n%s\n%s\n'%
            (inner_len,  inner_start, inner_stop, 'Power (dBm'))
    metafile.write('#outer loop\n%s\n%s\n%s\n%s\n'%
            (outer_len, outer_stop, outer_start, 'Frequency (GHz)'))
...
def awg_program():
    awg_program_string = """
    const control_power = 0.36;// 0.36
    const cycle = 4.4e-9;
    const us = 1e-6;
    const ns = 1e-9;
    const len = 133; //120
    const phase = 1.562237;
    const measure_pulse_length = 10 ; // us
    //Define the frequency.
    wave w1 = sine(1800, 0.95*0.993, phase, 33);
    wave w2 = sine(1800, 0.95*1.0, 0    , 33);
    //Define the envelope.
    wave w_gauss = gauss(128,control_power, 64, 16);
    //Cut the sine wave.
    wave w_cut1 = cut(w1, 0,127);
    wave w_cut2 = cut(w2, 0,127);
    //Multiply both sin and envelope.
    wave w_play1 = multiply(w_cut1, w_gauss);
    wave w_play2 = multiply(w_cut2, w_gauss);
    while (true) {
...
#Outerloop  
outer_start = 7.40714*GHz
outer_stop  = 7.40714*GHz
outer_len = 1
outer_meta_info = "Freq" 
...
aps.set_frequency(outer_start - Mix_freq)
smf.rf_on()
aps.rf_on()
channels = [1,2]
...
delay_progress_bar = progressbar.ProgressBar(maxval=inner_len, \
...
for outer_index, outer_value in enumerate(outerloop):
    ...
    for inner_index, inner_value in enumerate(innerloop):
        smf.set_source_power(inner_value)
        rte.reset_averages()
        rte.run_nx_single(wait=True)
        time_array, voltages = rte.get_data(channels)
        temp = (voltages[0]**2+voltages[1]**2)**0.5
        trace.append(sum(temp[240:340])/dBm_to_mVolt(inner_value))
        delay_progress_bar.update(inner_index+1)
