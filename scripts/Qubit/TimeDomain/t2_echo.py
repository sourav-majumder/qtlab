...
data_file_name = 't2_echo'#raw_input('Enter name of data file: ')
...
outer_len = 176
outer_meta_info = "Half-Gap (samples)"
...
if i_should_optimize:
    ...
    uhf.set('awgs/0/outputs/0/amplitude', 58/100.*max_i_amp)
    uhf.set('awgs/0/outputs/1/amplitude', 58/100.*max_q_amp)
# phase = -1.710238;
...
uhf.setup_awg(awg.t2_echo(phase,gap = 0))
uhf.awg_on(single=False) 
...
for outermost_index, outermost_value in enumerate(outermostloop):
    ...
    for outer_index, outer_value in enumerate(outerloop):
        ...
        uhf.setup_awg(awg.t2_echo(phase,gap=outer_value))
        uhf.awg_on(single=False)
