...
data_file_name = 't2'#raw_input('Enter name of data file: ')
...
outer_start = 0
outer_stop  = 10*us/(4.4*ns) 
outer_len = 176
outer_meta_info = "pulse length (samples)"
...
i_should_optimize = False

if i_should_optimize:
    i_dc, q_dc, phase, max_i_amp, max_q_amp = optimize(outermost_start)
else:
    phase = -1.801501#float(raw_input('Please enter last used phase : '))
    max_i_amp = uhf.get('awgs/0/outputs/0/amplitude')
    max_q_amp = uhf.get('awgs/0/outputs/1/amplitude')
    uhf.set('awgs/0/outputs/0/amplitude', 28/100.*max_i_amp)
    uhf.set('awgs/0/outputs/1/amplitude', 28/100.*max_q_amp)
# phase = -1.710238;
...
uhf.setup_awg(awg_program(phase, gap=0))
uhf.awg_on(single=False) 
...
for outermost_index, outermost_value in enumerate(outermostloop):
    ...
    for outer_index, outer_value in enumerate(outerloop):
        uhf.setup_awg(awg_program(phase = phase, gap=outer_value))
        uhf.awg_on(single=False)
