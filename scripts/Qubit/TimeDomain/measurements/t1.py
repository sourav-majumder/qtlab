...
def awg_program(delay):
    ...
    wave wgl = gauss(32, 1, 32, 8);
    wave wgr = gauss(32, 1, 0, 8);
    wave flat = rect(len, 1);
    wave w1 = join(wgl, flat, wgr);
    //sine(samples, amplitude, phaseOffset, nrOfPeriods)
    wave w2 = sine(64*320, 0.9, phase, 6*320);
    wave w3 = sine(64*320, 0.9, 0, 6*320);
    wave p1 = multiply(w1, w2);
    wave p2 = multiply(w1, w3);
    wave p1_play = cut(p1, 0, 32+len+32-1);
    wave p2_play = cut(p2, 0, 32+len+32-1);
    while (true) {
      playWave(p1_play, p2_play);
      waitWave();
      wait(14);
      wait(maadi);
      setTrigger(0b0010);
...
outermost_start = 6.63387*GHz
outermost_stop  = 6.63387*GHz  # One more than the end point you 
outermost_len  =  1
outermost_meta_info = "Frequency (Hz)" 

#Outerloop is delay between measure and control (cycles) 
outer_start = 0
outer_stop  = 10*us/(4.4*ns)  # 4.4*3 us 
outer_len = 31
