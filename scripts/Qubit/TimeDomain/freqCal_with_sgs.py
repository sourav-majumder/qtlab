...
def awg_program(points):
    ...
    const us = 1e-6;
    const ns = 1e-9;
    const power = 123.0/750.0;
    wave w_rise = -1*gauss(128,power, 128, 32);
    wave w_fall = -1*gauss(128,power, 0, 32);
    wave w_flat = -1*rect(1000, power);
    wave w_zer = zeros(200);
    wave w_join = join(w_zer,w_rise ,w_flat, w_fall);
    wave w1 = gauss(128, control_power, 64, 32);
    wave w_zero = zeros(delay);
    wave w_1 = join(w_zero, w1);
...
outer_start = 7.61441*GHz + 10*MHz
outer_stop  = 7.61441*GHz - 10*MHz
outer_len = 21
