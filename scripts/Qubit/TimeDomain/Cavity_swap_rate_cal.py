...
def awg_program(delay):
    awg_program_string = """
    const control_power = 0.45;
    const length = 640;
    const flux_point = 341;
    const flux_power = (flux_point-132.0)/750;
    const cycle = 4.4e-9;
    const us = 1e-6;
    const ns = 1e-9;
    wave w1 = gauss(128, control_power, 64, 32);
    wave w_rise = gauss(32, flux_power, 32, 8);
    wave w_fall = gauss(32, flux_power, 0, 8);
    wave w_flat = rect(length, flux_power);
