...
def awg_program(gap, ph = ph):
    ...
    wave w_join = join(w_gauss , w_gauss);
    //Cut the sin wave as the envelope length.
    wave w_cut1 = cut(w1, 0, 255);
    wave w_cut2 = cut(w2, 0, 255);
    //Multiply both sin and envelope.
    wave w_play1 = multiply(w_cut1, w_join);
    wave w_play2 = multiply(w_cut2, w_join);
    //Final
    wave w_use1 = join(cut(w_play1, 0, 127), w_zeros,cut(w_play1, 128, 255));
    wave w_use2 = join(cut(w_play2, 0, 127), w_zeros,cut(w_play2, 128, 255));
    while (true) {
