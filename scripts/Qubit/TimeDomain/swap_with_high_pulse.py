...
# def awg_program():
#     awg_program_string = """
#     const control_power = 0.46;
#     const cycle = 4.4e-9;
#     const us = 1e-6;
#     const ns = 1e-9;
#     const power = (222.0+123.0)/750.0;
#     wave w_rise = gauss(128,power, 128, 32);
#     wave w_fall = gauss(128,power, 0, 32);
#     wave w_flat = rect(20000, power);
#     wave w_join = join(w_rise ,w_flat, w_fall);
#     wave w1 = gauss(128*4, control_power, 64*4, 32*4);
#     wave w_zero = zeros(150);
#     wave wjoin = join(w_zero, w1);
...
def awg_program_2(control_length):
    awg_program_string = """
    const control_power = 1;
    const length = %d;
    const flux_power = (214.0-14.5)/750.0;
    const cycle = 4.4e-9;
    const us = 1e-6;
    const ns = 1e-9;
    const len  = 95;
    ...
    wave w_join = join(w_rise ,w_flat, w_fall);
    wave w_rise1 = gauss(8, flux_power, 8, 1);
    wave w_fall1 = gauss(8, flux_power, 0, 1);
...
for outermost_index, outermost_value in enumerate(outermostloop):
    ...
    for outer_index, outer_value in enumerate(outerloop):
        # sgs.set_frequency(outer_value)
        uhf.setup_awg(awg_program_2(outer_value))
        uhf.awg_on(single=False)
