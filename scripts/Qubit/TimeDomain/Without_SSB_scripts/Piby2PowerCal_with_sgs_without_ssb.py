...
def awg_program(power):
    ...
    wave w_flat = rect(250, control_power);
    wave w_gauss = join(w_rise ,w_flat, w_fall);
    wave w_wave = join(w_gauss, w_gauss);
    while (true) {
      playWave(w_wave);
      waitWave();
      wait(14);
