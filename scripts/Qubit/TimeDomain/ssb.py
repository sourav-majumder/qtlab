...
def optimize_dc(plot = False):
	...
	i_arr = np.linspace(-200e-3, 200e-3, 25) # -18.49m
	q_arr = np.linspace(-200e-3, 200e-3, 25) # -24.24m
...
def awg_program(phase=0):
	...
	wave w2 = sine(128, 0.7, %f, 12);
	wave w3 = sine(128, 0.7, 0, 12);	
	while (true) {
	  playWave(w2, w3);
	}""" % \
	(phase)
...
def optimize_phase(plot=False):
	ph_arr = np.linspace(-np.pi, np.pi, 81)
	opt_ph = optimize_ph(ph_arr, plot)
	# opt_ph = -1.75
	# Fine sweep
	ph_arr = np.linspace(0.96*opt_ph, 1.04*opt_ph, 21)
	opt_ph = optimize_ph(ph_arr, plot)
	# Finer sweep
...
def optimize_amp_update(q_arr, plot=False):
	...
	progress_bar = progressbar.ProgressBar(maxval=len(q_arr), \
	    widgets=['Optimizing Amplitude', progressbar.Bar('.', '', ''), ' ', progressbar.Percentage(), ' (', progressbar.ETA(), ') '])
	progress_bar.start()
	uhf.set('awgs/0/outputs/0/amplitude', base_i)
...
def optimize_amplitude_update(plot=False):
	q_arr = np.linspace(0.40, 0.60, 41)
	opt_q = optimize_amp_update(q_arr, plot)
	uhf.set('awgs/0/outputs/1/amplitude', opt_q)
	# Fine sweep

	q_arr = np.linspace(0.9*opt_q, 1.1*opt_q, 21)
	opt_q = optimize_amp_update(q_arr, plot)
	uhf.set('awgs/0/outputs/1/amplitude', opt_q)
	# Finer sweep

	q_arr = np.linspace(0.98*opt_q, 1.02*opt_q, 41)
	opt_q = optimize_amp_update(q_arr, plot)
	uhf.set('awgs/0/outputs/1/amplitude', opt_q)
	return opt_q
...
def optimize_all_update(plot=False):
	...
	fsv.set_markerN_frequency(3, center_freq)
