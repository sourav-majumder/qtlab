...
def S21(f, f0, ke1, ke2, ki, norm):
	return norm*np.sqrt(ke1*ke2)/(-1j*(f-f0)+(ke1+ke2+ki)/2)

def S21r(f, f0, k, norm):
	return np.abs(norm/(-1j*(f-f0)+k/2))
...
def custom(f, f0, ke, ke1, ki, norm):
	return norm*np.abs((ki-2*1j*(f-f0))/(ki+ke+1j*ke1-2*1j*(f-f0)))

def custom_real(f, f0, ke, ki, norm):
	return norm*np.real((ki-2*1j*(f-f0))/(ki+ke-2*1j*(f-f0)))

def custom_imag(f, f0, ke, ki, norm):
	return norm*np.imag((ki-2*1j*(f-f0))/(ki+ke-2*1j*(f-f0)))

def omit_transmission(f,f0,norm, gamma, Cp):
    return np.abs( norm * (1. -  Cp / (-1j*(2*(f-f0)/gamma) + 1 + Cp )) )
...
omit_func_list = [omit_transmission]
...
all_funcs = lorentzian_func_list + inverted_lorentzian_func_list + single_port_func_list + omit_func_list + two_port_func_list
...
class Fitter(object):
	...
	def fit(self, xdata, ydata, print_report=False, use_previous_values=False, **kwargs):
		...
		elif self.func in omit_func_list:
			f0 = xdata[np.argmin(np.abs(y))]
			norm = np.max(np.abs(y))
			Cp = (np.max(y) - np.min(y)) /np.min(y)
			#gamma = approx_FWHM(xdata, ydata)/(1+Cp)
		...
		for param in self.params:
			...
			elif self.func in two_port_func_list:
				if param.name == 'f0':
					param.init = xdata[np.argmax(np.abs(y))]
				elif param.name =='k':
					param.init = 2e6
				elif param.name == 'norm':
					param.init = 0.9
			...
			init = param.init
			if param.name == 'ke1' or param.name == 'ki' or param.name =='ke2' or param.name == 'k':
				lmfit_params.add(param.name, value=param.init, min=0, max=param.stop)
			else:
				lmfit_params.add(param.name, value=param.init, min=param.start, max=param.stop)
		# print lmfit_params
