...
rigol_phase_start = 36
rigol_phase_stop = 37
rigol_phase_resolution = 1e-3
rigol_phase_numpoints = abs(rigol_phase_start-rigol_phase_stop)/rigol_phase_resolution + 1
rigol_phase_array = np.linspace(rigol_phase_start, rigol_phase_stop, rigol_phase_numpoints)
...
rig_inst = visa.ResourceManager().open_resource('USB0::0x1AB1::0x0642::DG1ZA194405642::INSTR')
raw_input('do_phase_align')
...
def generate_meta_file(no_of_phase, phase):
	metafile = open('%s.meta.txt' % data.get_filepath()[:-4], 'w')
	metafile.write('#inner loop\n%s\n%s\n%s\n%s\n'%
			(record_length,  start_time/us, stop_time/us, 'Time(us)'))
	metafile.write('#outer loop\n%s\n%s\n%s\n%s\n'%
			(no_of_phase, phase, rigol_phase_array[0], 'Hz'))
...
phase_progress_bar = progressbar.ProgressBar(maxval=rigol_phase_numpoints, \
	widgets=['Total: ', progressbar.Bar('=', '[', ']'), ' ', progressbar.Percentage(), ' (', progressbar.ETA(), ') '])
phase_progress_bar.start()

areas = []
for no_of_phase, phase in enumerate(rigol_phase_array):
	rig_inst.write('PHAS %f' % phase)
	rte.reset_averages()
	rte.run_nx_single(wait=True)
	phase_arr = np.linspace(phase, phase, record_length)
	time_array, voltages = rte.get_data(channels)
	voltages.append((voltages[0]**2 + voltages[1]**2)**0.5)
	data.add_data_point(phase_arr, time_array, *voltages)
	generate_meta_file(no_of_phase+1, phase)
	phase_progress_bar.update(no_of_phase+1)
