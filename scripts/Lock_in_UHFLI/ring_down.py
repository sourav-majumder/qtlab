...
awg_sample_rate = 1.8e9
frequency = 1e6

def freq_to_samples(freq):
	return int(awg_sample_rate/float(frequency))
...
out_channel = 0
out_mixer_channel = zhinst.utils.default_output_mixer_channel(props)#3
in_channel = 0
osc_index = 0
awg_channel = 0
demod_index = 0
...
num_pulses = pulse_length*frequency
...
awg_program = awg_program.replace('_c1_', '%d' % samples)

awg_program = awg_program.replace('_num_', '%d' % num_pulses)
...
daq.setInt('/%s/scopes/0/length' % device, int(measurement_length))
...
assert trigger.finished(), "Trigger not finished"
...
for i in range(0, len(samples)):
	...
	# dictionary.
	t = (samples[i]['timestamp'] - float(samples[i]['time']['trigger']))/clockbase
